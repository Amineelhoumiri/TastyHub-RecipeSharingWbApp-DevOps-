...
describe('Recipe Endpoints', () => {
  ...
  it('POST /api/recipes - should reject without title', () => {
    ...
    cy.authenticatedRequest('POST', '/api/recipes', recipeData, { failOnStatusCode: false }).then((response) => {
  ...
  it('POST /api/recipes/:recipeId/comments - should reject comment without text', () => {
    ...
    cy.authenticatedRequest('POST', `/api/recipes/${testRecipeId}/comments`, { rating: 5 }, { failOnStatusCode: false }).then((response) => {
      expect(response.status).to.eq(400);
